...
// Register needed chart types/plugins once
...
async function getDailyHistory(providerId?: string) {
  const today = dayjs();
  const weekStart = today.startOf("week").format("YYYY-MM-DD"); // starts on Sunday
...
function freshCanvas(selector: string | HTMLElement): HTMLCanvasElement {
  ...
  // Let Chart.js do responsive sizing, but clamp so it can't exceed its parent.
        ...
        tooltip: {
          callbacks: {
            label(context) {
              ...
              const displayTime = renderTimeSynchronously(formatTime(valueMs), true); // always show seconds we have no way to determine it
...
async function getChartDataByViewType(viewType: Views): Promise<number[]> {
  ...
  switch (viewType) {
    case "weekly":
      data = await getDailyHistory();
      break;
    case "monthly":
      data = await getWeeklyHistory(MAX_WEEKS_TO_KEEP);
      break;

    case "yearly":
      data = await getMonthlyHistory();
      break;

    case "alltime":
      data = await getYearlyHistory();
      break;
  }
  if (data) {
...
async function renderUsageChart(viewType: Views): Promise<void> {
  ...
  barChart = new ChartJS(canvas, {
    ...
    data,
    options: {
      responsive: true, // Chart.js handles width
      ...
      scales: {
        x: { grid: { display: false } }, // Override the scale
        y: {
          beginAtZero: true,
          ticks: {
            callback(value) {
